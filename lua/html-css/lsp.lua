...
-- Local override for `html-css.lsp` to improve TSX/JSX context detection by
-- resolving nodes from active parser first, then language fallbacks.
-- This file intentionally shadows plugin module `html-css/lsp.lua`.
...
local function get_context_node(position)
    local pos = { position.line, position.character }

    -- Prefer the active parser tree for the current buffer/filetype.
    local node = vim.treesitter.get_node({ pos = pos })
    if node then
        return node
    end

    -- Fallbacks for React-like buffers where parser resolution can vary.
    return vim.treesitter.get_node({ lang = "tsx", pos = pos })
        or vim.treesitter.get_node({ lang = "javascript", pos = pos })
        or vim.treesitter.get_node({ lang = "html", pos = pos })
end
...
local function create_server(dispatchers)
    ...
    function srv.request(method, params, callback)
        ...
        elseif method == "textDocument/completion" then
            ...
            local context = nil
            local node = get_context_node(position)
...
LSP.create_client = function(opts, bufnr)
    ...
    local root_marker = vim.fs.find({ ".git", "package.json", ".nvim.lua" }, { upward = true })[1]
